    ...
    def __call__(
        ...
        for i, t in enumerate(timesteps):
            ...
            if do_classifier_free_guidance:
                ...
                #print(torch.nn.functional.mse_loss(noise_pred_uncond, noise_pred_text).item())
...
class analysis_Inversion(Inversion):
    ...
    def loop(self, latent, guidance_scale):
        ...
        for i in range(total_steps):
            ...
            ################ optimization steps #################
            if self.method == 'afpi':
                latent, dist_loss = self.afpi_step(latent, latent_ztm1, t, guidance_scale)
            elif self.method == 'aidi':
                latent, dist_loss = self.aidi_step(latent, latent_ztm1, t, guidance_scale)
            else:
                latent, dist_loss = self.fpi_step(latent, latent_ztm1, t, guidance_scale)
            dist_losses.append(dist_loss)
            all_latent.append(latent)
        return all_latent, dist_losses

    def invert(self, image_latent, prompt: str, guidance_scale):
        self.init_prompt(prompt)
        all_latent, dist_losses = self.loop(image_latent, guidance_scale)
        return all_latent, dist_losses
